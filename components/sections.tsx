...
export function ScheduleView({ schedule }: { schedule: Schedule }) {
  const { days, legend } = schedule;
  const byColor = new Map<string, LegendEntry>();
  for (const l of legend) if (l.bg && !byColor.has(l.bg)) byColor.set(l.bg, l);
        ...
        <div className="legend">
          {legend.map((l, i) => (
            <span className="legend-item" key={i}>
              <span
                className="swatch"
                style={{ background: l.bg || "transparent" }}
                aria-hidden
              />
              {l.label}
            </span>
            ...
            <div className={`day${isThanksgiving ? " day-thanksgiving" : ""}`} key={i}>
              ...
              <div className="events">
                {d.events.length === 0 ? (
                  ...
                  d.events.map((e, j) => {
                    const cat = e.bg ? byColor.get(e.bg) : undefined;
    ...
    <div className="grid three">
      {items.map((r, i) => (
        <div className="act" key={i}>
          ...
          {r.location && <div className="stay-meta"><span className="pill">{r.location}</span></div>}
          {r.notes && <div className="act-notes">{r.notes}</div>}
          <div className="map-link">
            <a href={mapsUrl(`${r.name} restaurant Portland Oregon`)} target="_blank" rel="noreferrer">
              Map
            </a>
          </div>
        </div>
